...
where $F, G\colon\ \mathbb{R} \to \left[0, 1\right]$ are the cumulative distribution functions of the underlying unknown continuous distributions. Let $G$ be a shifted version of $F$, i.e.
...
`m_test`      | M-estimator test | Tests based on M-estimators | 
...
\begin{align*}
  \hat{S}^{(4)} = \text{median}\left\{|x_1 - \tilde{x}|, \ldots, |x_m - \tilde{x}|\right\} + 1.4826 \text{median}\left\{|y_1 - \tilde{y}|, \ldots, |y_n - \tilde{y}|\right\}.
\end{align*}
...
In our package, we use the $\tau$-scale estimator as implemented in the package [robustbase](https://cran.r-project.org/web/packages/robustbase/index.html) [@MaeRouCro20robu].
...
We currently allow for Huber-, Hampel-, and Bisquare $\psi$-functions called from [robustbase](https://cran.r-project.org/web/packages/robustbase/index.html).
...
Instead of computing all possible splits, only a random subset of $b << B$ splits is selected. 
...
Following [@PhiSmy10perm], we correct the $p$-value using the function `permp` from their R package [statmod](https://cran.r-project.org/web/packages/statmod/index.html).

### Asymptotic
For very large sample sizes, asymptotical $p$-values can be computed for each test using a normal approximation. This reduces the computation time further as compared to the randomization principle.
...
We use the R function `densitiy` from the [stats](https://cran.r-project.org/web/packages/stats/index.html) package with its default settings for the kernel-density estimation.
