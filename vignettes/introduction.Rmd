...
\begin{align*}
  ...
  &H_0^{(>)}\colon\ \Delta \leq \Delta_0 \quad \text{vs.} \quad H_1^{(>)}\colon\ \Delta \neq \Delta_0 \\
  &H_0^{(<)}\colon\ \Delta \geq \Delta_0 \quad \text{vs.} \quad H_1^{(<)}\colon\ \Delta \neq \Delta_0.
\end{align*}
...
### MED tests
An obvious choice to obtain a robustified version of the $t$-statistic would be
to replace the sample means by the sample medians, i.e. the location difference
is estimated by
...
Currently, two different options for estimating the within-sample variability are
available. Setting the argument `scale = S3`, the scale is estimated by the median
of the absolute deviations of each observation from its corresponding sample median:
...
and efficiency are the Hodges-Lehmann estimators (Hodges & Lehmann, 1964).
...
\begin{align*}
   \hat{\Delta}^{(\text{HL2})} = \text{median}\left\{|x_i - y_j|\colon\ 1 \leq i \leq m,\ 1 \leq j \leq m\right\}.
\end{align*}
...
included multiple times in the drawn splits. Following Smyth & Phipson (2011),
...
We implemented a procedure call wobbling (Fried & Gather, 2007), i.e. we add random noise from a uniform distribution to the
...
Following Fried (2012), the argument `var.test` allows to decide whether the
