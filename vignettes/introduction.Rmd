...
cannot be justified. However, it can be nearly as vulnerable to outliers [@FriGat07rank].
...
In addition to the two-sample location problem, the tests can be used to
...
replace the sample means, as used in the standard $t$-statistic, by the sample medians. I.e., the location difference
is estimated by
...
The test can be called with the function `med_test`.
...
An M-estimator $\hat{\mu}^{(M)}_X$ for the locatin parameter of the $x$-sample (and analogously for the $y$-sample) can be obtained by solving the minimization problem
\begin{equation*}
  \hat{\mu}^{(M)}_X = \arg\underset{\mu_X}{\min} \sum_{i = 1}^m \rho\left(\frac{x_i - \mu_X}{\sigma_X}\right),
\end{equation*}
where $\rho:\ \mathbb{R} \to \mathbb{R}$ is a function which is chose to achieve a \enquote{nearly optimal} location estimate when the $X_i$ are normally or approximately normally distributed [@MarMarYoh06robu, p. 22].
...
For an increasing function $\psi$, it can be shown that
\begin{equation*}
...
To estimate $\nu_X$, we replace the expected values by the sample means, i.e.
\begin{equation*}
...
The only estimator $\hat{\sigma}_X$ for $\sigma_X$ which ensures the asymptotical normal distribution stated before, is the empirical standard deviation, which, however, is not robust.
In the package, we decided to use the $\tau$-scale estimator as implemented in the package [robustbase](https://cran.r-project.org/web/packages/robustbase/index.html) [@MaeRouCro20robu].
...
However, it becomes worse for very skewed distributions, under which the resulting two-sample tests becomes conservative.
Hence, larger samples are necessary for an acceptable approximation.

The test statistic of the two-sample $M$-estimator test in the package is
\begin{equation*}
  \sqrt{\frac{m \cdot n}{n \cdot \hat{\sigma}^2_X \cdot \hat{\nu}_X + m \cdot \hat{\sigma}^2_Y \cdot \hat{\nu}_Y}} \cdot \hat{\mu}^{(M)}_X - \hat{\mu}^{(M)}_Y.
\end{equation*}

We currently allow for Huber-, Hampel-, and Bisquare $\psi$-functions as implemented in [robustbase](https://cran.r-project.org/web/packages/robustbase/index.html).

## Computation of p-values
The $p$-values for the different tests can be computed in various ways, which are
...
### Permutation test
Using the permutation principle, the $p$-value is obtained by computing the value of the test statistic for all possible $B = \binom{m}{m + n}$ splits of the joint sample $\left(x_1, \ldots, x_m, y_1, \ldots, y_n\right)$ into two samples of sizes $m$ and $n$.
The underlying idea is that all splits occur with equal probability under $H_0$. 

The permutation principle allows for an exact computation of the $p$-value and leads to distribution-free tests, i.e. a permutation test holds its significance level under every distribution.

The $p$-value corresponds to the fraction of computed values of the test statistic, which are at least as extreme as the observed value, represented by $A$ in the following.
Let $t_i$ be the value of the test statistic for split $i$, $i = 1, \ldots, B$, and $t^{(\text{obs})}$ the observed value of the test statistic. Then,
...
### Randomization
The randomization principle can be used to deal with the aforementioned shortcomings of the permutation principle. 
Instead of computing all possible splits, only a random subset of $b << B$ splits is selected. As commonly proposed
in the literature, we draw these random subsets with replacement, i.e. a single permutation may be drawn repeatedly.
...
This estimator will overestimate the true $p$-value because the observed sample may be drawn several times. Following [@PhiSmy10perm],
we correct the $p$-value using their function `permp` from the R package [statmod](https://cran.r-project.org/web/packages/statmod/index.html).

### Asymptotic
For very large sample sizes, asymptotical $p$-values can be computed for each test. This further reduces the computation time as compared to the randomization principle.
...
sample $x_1 - \tilde{x}, \ldots, x_m - \tilde{x}, y_1 - \tilde{y}, \ldots, y_n - \tilde{y}$. We use the R function
`densitiy` from the [stats](https://cran.r-project.org/web/packages/stats/index.html) package with its default settings for the kernel-density estimation.
...
\end{align*}
where $\int_{-\infty}^{\infty} f^2\left(x\right) \mathrm{d}x$ can be interpreted as the value of the density of $X - Y$ at zero. In practice, it can be estimated by a kernel-density estimator on the within-sample pairwise differences
...
#### Asymptotic M-test
As stated before, the test statistic of the two-sample $M$-test follows an asymptotical normal distribution under certain assumptions.
...
## Continuous data
Let $x_1, \ldots, x_m$ and $y_1, \ldots, y_m$ be observations from a continuous distrubtions.
...
The $p$-value can be accessed via `hl1.res$p.value`. To extract the value of the test statistic, we use `hl1.res$statistic`, and for the HL1-estimates of each sample `hl1.res$estimate`.

### Randomization test
In this example, we draw 10000 splits randomly from the observed joint sample to perform a randomized HL1-test.
...
The $p$-value is close to the one we have calculated using the permutation principle.

### Asymptotic test
Although the sample sizes in our example are rather small, we use the samples to demonstrate how to use the asymptotic HL1-test.
...
In this example, we use $\Delta_0 = 5$, i.e. under $H_0$, we assume that the location parameter of the distribution of $X_i$, $i = 1, \ldots, m$, is five units larger than that of the distribution of $Y_j$, $j = 1, \ldots, m$.
...
Following the suggestion of [@FriGat07rank], we implemented a procedure called wobbling, i.e. we add random noise from a uniform distribution to the observations. 
To keep the alterations small, the scale of the noise depends on the number of decimal places of the observations.

The addition of random noise is controlled by the argument `wobble`, where `TRUE` means that noise is added and `FALSE` means that no noise is added.
...
In terms of the power, the resulting variance tests can outperform classical tests like the $F$-test,
the Mood test, or the Ansary-Bradley test under outliers and asymmetry.
...
## Other packages
The tests in this package are constructed assuming homoscedasticity. Robust two-sample tests for the heteroscedastic scenario can be found in R package [WRS2](https://cran.r-project.org/web/packages/WRS2/index.html) [@MaiWil20robu].
