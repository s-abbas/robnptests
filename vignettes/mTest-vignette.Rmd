...
For a more thoroughly description of the asymptotic behaviour of M-estimators, we refer to @MarMarYoh06robu[p. 36ff.], which we use as the main reference for the following motivation.
...
For M-estimators $\hat{\mu}_X$ and $\hat{\mu}_Y$ with a $\psi$-function $\psi$ and $\rho$-function $\rho = \psi'$, it can be shown that
\begin{align*}
...
\begin{align*}
\nu_X = \frac{\text{E}\left(\psi\left(\frac{X - \mu_X}{\sigma_X}\right)^2\right)}{\left(\text{E}\left(\rho\left(\frac{X - \mu_X}{\sigma_X}\right)\right)\right)^2} \quad \text{and} \quad
\nu_Y = \frac{\text{E}\left(\psi\left(\frac{Y - \mu_Y}{\sigma_Y}\right)^2\right)}{\left(\text{E}\left(\rho\left(\frac{Y - \mu_Y}{\sigma_Y}\right)\right)\right)^2}.
\end{align*}
...
In order to use this statistic as a test statistic for our M-tests, we need to estimate $\sigma_X$ and $\sigma_Y$ as well as $\nu_X$ and $\nu_Y$.
We use the $\tau$-scale estimator [@MarZam02robu] to estimate $\sigma^2_X$ and $\sigma^2_Y$ by $\hat{\sigma}_X$ and $\hat{\sigma}_Y$ robustly and estimate $\nu_X$ and $\nu_Y$ by

\begin{align*}
\hat{\nu}_X = \frac{\frac{1}{m} \sum_{i = 1}^m \psi\left(\frac{X_i - \hat{\mu}_X}{\hat{\sigma}_X}\right)^2}{\left(\frac{1}{m} \sum_{i = 1}^m \rho\left(\frac{X_i - \hat{\mu}_X}{\hat{\sigma}_X}\right)\right)^2} \quad
\text{and} \quad
\hat{\nu}_Y = \frac{\frac{1}{m} \sum_{i = 1}^m \psi\left(\frac{Y_i - \hat{\mu}_Y}{\hat{\sigma}_Y}\right)^2}{\left(\frac{1}{m} \sum_{i = 1}^m \rho\left(\frac{Y_i - \hat{\mu}_Y}{\hat{\sigma}_Y}\right)\right)^2}.
\end{align*}

Within the function `m_test`, we use the function `scaleTau2` from the `R` package [robustbase](https://cran.r-project.org/package=robustbase) [@MaeRouCro20robu].

Hence, the test statistic used in `m_test` is given by
...
# Simulation results
The following figure shows the simulated test sizes from a small simulation study with 1000 replications, where we applied the M-tests with different $\psi$-functions to samples from the $\mathcal{N}(0, 1)$-distribution, the $t_2$-distribution, and the $\chi^2_3$-distribution. We chose the significance level $\alpha = 0.05$. 

The results under the $\mathcal{N}(0, 1)$- and the $t_2$-distribution are similar: 
For equal sample sizes $m = n \geq 30$, the simulated test size is quite close to the the specified value of $\alpha$. When $m \neq n$, it seems to be important that both values are rather large do not deviate too much from each other.
Otherwise, the tests may become very anti-conservative. 
In general, the three test statistics lead to similar results for the considerated sample sizes.
