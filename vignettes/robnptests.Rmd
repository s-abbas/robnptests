...
The argument `disp.test` allows to decide whether the samples should be tested for a location difference (`disp.test = FALSE`), which is the default, or for different scale parameters (`disp.test = TRUE`).

As described in the introduction, setting `disp.test = TRUE` transforms the observations so that a possible scale difference between the samples appears as a location difference between the transformed samples. For these tests, the samples need to be centred beforehand.
...
hl1_test(x = x, y = y, alternative = "two.sided", delta = 1, method = "asymptotic", disp.test = TRUE)
#> 
#> 	Asymptotic test based on HL1-estimator
#> 
#> data:  x and y
...
hl1_test(x = x * sqrt(5), y = y, alternative = "two.sided", delta = 5, method = "asymptotic", disp.test = TRUE)
#> 
#> 	Asymptotic test based on HL1-estimator
#> 
#> data:  x * sqrt(5) and y
