...
\begin{align*}
  &H_0^{(=)}\colon\ \frac{\sigma^2_X}{\sigma^2_Y} = \tilde{\Delta}_0^2 \quad \text{vs.} \quad H_1^{(\neq)}\colon\ \frac{\sigma^2_X}{\sigma^2_Y} \neq \tilde{\Delta}_0^2  \\
  &H_0^{(>)}\colon\ \frac{\sigma^2_X}{\sigma^2_Y} \leq \tilde{\Delta}_0^2 \quad \text{vs.} \quad H_1^{(>)}\colon\ \frac{\sigma^2_X}{\sigma^2_Y} > \tilde{\Delta}_0^2 \\
  &H_0^{(<)}\colon\ \frac{\sigma^2_X}{\sigma^2_Y} \geq \tilde{\Delta}_0^2 \quad \text{vs.} \quad H_1^{(<)}\colon\ \frac{\sigma^2_X}{\sigma^2_Y} < \tilde{\Delta}_0^2.
\end{align*}

By log-transforming the random variables, we obtain

\begin{align*}
  & U_i = \log\left(X_i^2\right) = \log\left(\sigma_X^2\right) + \log\left(\varepsilon_{ i}^2\right),\ i = 1, \ldots, m, \\ \text{and }  \quad & V_i = \log\left(Y_j^2\right) = \log\left(\sigma_Y^2\right) + \log\left(\varepsilon_{m+j}^2\right),\ j = 1, \ldots, n.
\end{align*}

With these transformed samples, we are again in the data situation described above as the distributions differ at most by  \[ \Delta_0 = \log(\tilde{\Delta}_0) = \log\left(\sigma_X^2\right) - \log\left(\sigma_Y^2\right)\] [see @Fri12onli].
...
#>  [6] backports_1.2.1   magrittr_2.0.1    evaluate_0.14     Rdpack_2.1.1      rlang_0.4.11     
#> [11] stringi_1.4.6     checkmate_2.0.0   rmarkdown_2.8     statmod_1.4.36    tools_4.0.2      
#> [16] stringr_1.4.0     rsconnect_0.8.18  yaml_2.2.1        xfun_0.23         compiler_4.0.2   
