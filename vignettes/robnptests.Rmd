...
Thus, $\boldsymbol{X}_i$ and $\boldsymbol{Y}_j$ are from the same location-scale family.
...
When setting $\mu_X = \mu_Y = 0$, both data-generating distributions have the location parameter zero but the values of the scale parameter might be unequal.
We address the following hypothesis pairs:
\begin{align*}
...
We show an example of how use them to test for scale differences at the end of this vignette.
...
* (approximate) distribution independence, i.e. keep $\alpha$ under $H_0$ over a large class of continuous distributions,
...
The argument `scale_test` allows to decide whether the samples should be tested for a location difference (`scale_test = FALSE`), which is the default, or for different scale parameters (`scale_test = TRUE`).

As described in the introduction, setting `scale_test = TRUE` transforms the observations so that a possible scale difference between the samples appears as a location difference between the transformed samples. For these tests, the samples need to be centred beforehand.
...
## Asymptotic two-sided test assuming equal scales for both samples
hl1_test(x = x, y = y, alternative = "two.sided", delta = 1, method = "asymptotic", scale_test = TRUE)
#> 
#> 	Asymptotic test based on HL1-estimator
#> 
#> data:  x and y
...
```r
## Asymptotic two-sided test assuming a scale ratio of 5 between both samples
hl1_test(x = x * sqrt(5), y = y, alternative = "two.sided", delta = 5, method = "asymptotic", scale_test = TRUE)
#> 
#> 	Asymptotic test based on HL1-estimator
#> 
#> data:  x * sqrt(5) and y
...
If $\mu_X \neq 0$ or $\mu_Y \neq 0$, we need to estimate these parameters from the samples. They can then be subtracted from the respective sample to achieve the required centring.

In the following example, the second sample is centred around 5 instead of 0. The test erroneously rejects the null hypothesis.
...
By centring the samples using a suitable mean estimator, we can avoid this behavior.
...
## Asymptotic two-sided test with non-centred variables
hl1_test(x = x_centred, y = y_centred, alternative = "two.sided", delta = 1, method = "asymptotic", scale_test = TRUE)
#> 
#> 	Asymptotic test based on HL1-estimator
#> 
...
The choice of the location estimator depends on the distribution of the variables, and also on the desired robustness.
