...
\end{align*}
Here, $\mu_X$ and $\mu_Y$ denote the location parameters, and $\theta_X$ and $\theta_Y$ are the scale parameters of the first and the second sample. The random variables $\varepsilon_{1}, \ldots, \varepsilon_{m+n}$ are i.i.d. random variables. Thus, $\boldsymbol{X}_i$ and $\boldsymbol{Y}_j$ are from the same location-scale family.

### Differences in location

Assuming $\theta_X = \theta_Y$, the distributions of the samples $\boldsymbol{X}$ and $\boldsymbol{Y}$ differ at most in location. We denote the location difference by $\Delta = \mu_X - \mu_Y$.
Writing
\begin{align*}
...
With these transformed samples, we are again in the data situation described above as the distributions differ at most by  \[ \Delta_0 = \log(\tilde{\Delta}_0) = \log\left(\theta_X^2\right) - \log\left(\theta_Y^2\right)\] [see @Fri12onli].

### Testing for differences in location or scale

A popular test for the location scenario described above is the ordinary two-sample $t$-test, which uses the normality assumption. For large samples, the central limit theorem ensures that the test keeps the desired significance level $\alpha \in \left(0, 1\right)$ at least approximately, even if the underlying distributions are not normal. However, relying on the central limit theorem is often not appropriate in small samples. Also, there are distributions without existing variance for which the central limit theorem does not apply.
...
The implemented test statistics follow the construction principle of the $t$-statistic, i.e. an estimator $\hat{\Delta}$ for the true location difference $\Delta$ between both samples is divided by a pooled estimate $\hat{S}$ of the variability of the two samples, leading to test
...
For a motivation on how this test statistic is constructed, we refer to the vignette [\underline{Construction of the M-tests}](mTest-vignette.pdf). Similar test statistics were proposed in @YueLee85Some and @Abo92robu.
...
y
#>  [1] -0.8130825  1.3807136 -0.4387136 -1.2788213 -0.6058519  0.9234760  0.5656956 -0.0586360
#>  [9]  0.1578270 -0.3858708
```
...
#> $x
#>  [1] -0.44559409  0.25924234  0.18034432 -0.42507561 -0.07607574  0.68780214  0.17812437
#>  [8] -0.22174378  0.19595295 -0.14095250
#> 
#> $y
#>  [1] -0.04372255  2.22220106  1.23878909  0.16253680 -0.13902692  2.43266503  1.86621893
#>  [8]  1.41821762  0.71976748  1.36813320
```

# Testing for a difference in scale

The argument `var.test` allows to decide whether the samples should be tested for a location difference (`var.test = FALSE`), which is the default, or for different scale parameters (`var.test = TRUE`).
...
Including a scale ratio of 5 yields

```r
## Asymptotic two-sided test assuming a scale ratio of 5 between both samples
hl1_test(x = x * sqrt(5), y = y, alternative = "two.sided", delta = 5, method = "asymptotic", var.test = TRUE)
#> 
#> 	Asymptotic test based on HL1-estimator
#> 
#> data:  x * sqrt(5) and y
...
#> [1] robnptests_0.1.0 robTests_0.1.0  
#> 
#> loaded via a namespace (and not attached):
#>  [1] codetools_0.2-16 gtools_3.8.2     packrat_0.6.0    digest_0.6.25    rbibutils_2.1.1 
#>  [6] backports_1.2.1  magrittr_2.0.1   evaluate_0.14    Rdpack_2.1.1     stringi_1.4.6   
#> [11] checkmate_2.0.0  statmod_1.4.36   tools_4.0.2      stringr_1.4.0    xfun_0.23       
#> [16] compiler_4.0.2   knitr_1.33
```
