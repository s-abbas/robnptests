...
We implemented tests proposed in the literature for the outlined situation with the following objectives in mind:
...
For a motivation on how this test statistic is constructed, we refer to the vignette [\underline{Construction of the M-tests}](mTest-vignette.html). Similar test statistics were proposed in @YueLee85Some and @Abo92robu.
...
### Asymptotic test
For large sample sizes ($m, n>30$), it may be advisable to compute asymptotic $p$-values using a normal approximation to reduce the computation time further.
Especially in situations, where the tests have to be applied repeatedly to several data sets within a certain time frame, this reduces the computational burden.
...
x
#>  [1] -0.112892122 -0.381819373 -0.091691759  0.036045206  0.002060011  1.254923407  0.589883973  0.302208595
#>  [9]  0.905741048  0.054724887
y
...
If `check = TRUE` and all values are unique, the wobbling is ommitted.
...
As described in the introduction, setting `var.test = TRUE` transforms the observations so that a possible scale difference between the samples appears as a location difference between the transformed samples.
...
#> 
#> locale:
#> [1] LC_COLLATE=English_Austria.65001  LC_CTYPE=C                        LC_MONETARY=English_Austria.65001
#> [4] LC_NUMERIC=C                      LC_TIME=English_Austria.65001    
...
#>  [1] codetools_0.2-18  gtools_3.8.2      digest_0.6.27     rbibutils_2.2.1   backports_1.2.1  
#>  [6] magrittr_2.0.1    evaluate_0.14     Rdpack_2.1.2      rlang_0.4.10      stringi_1.5.3    
#> [11] robustbase_0.93-8 checkmate_2.0.0   rmarkdown_2.7     statmod_1.4.35    tools_4.0.4      
#> [16] stringr_1.4.0     DEoptimR_1.0-9    xfun_0.21         yaml_2.2.1        compiler_4.0.4   
