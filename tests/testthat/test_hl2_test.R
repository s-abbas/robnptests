testthat::test_that("hl2_test works correctly", {
  ...
  # Permutation test
  testthat::expect_snapshot_output(hl2_test(x = x[1:5], y = y[1:5],
  ...
  # Randomization test
  testthat::expect_snapshot_output(hl2_test(x = x[1:10], y = y[1:10],
  ...
  # Asymptotic test
  testthat::expect_snapshot_output(hl2_test(x = x, y = y, method = "asymptotic"))
  testthat::expect_snapshot_output(hl2_test(x = x, y = y, method = "asymptotic",
                                            var.test = TRUE))

  # Compare value of the test statistic to manually computed value ----

  res.s1 <- as.numeric(hl2_test(x = x, y = y, method = "randomization", n.rep = 100,
                                scale = "S1")$statistic)
  ...
  testthat::expect_equal(hl2_test(x = x, y = y)$method,
                         "Asymptotic test based on HL2-estimator")

  # Randomization test for small samples
  testthat::expect_equal(hl2_test(x = x[1:10], y = y[1:10], n.rep = 100)$method,
  ...
  # possible splits
  testthat::expect_equal(hl2_test(x = x[1:5], y = y[1:5], n.rep = 252)$method,
                         "Exact permutation test based on HL2-estimator")

  # User-specified selection of the method to compute the p-value ----

  # Asymptotic test
  testthat::expect_equal(hl2_test(x = x, y = y, method = "asymptotic")$method,
                         "Asymptotic test based on HL2-estimator")

  # Randomization test for small samples
  testthat::expect_equal(hl2_test(x = x[1:5], y = y[1:5], method = "randomization",
                                  n.rep = 100)$method,
                         "Randomization test based on HL2-estimator (100 random permutations)")
  ...
  testthat::expect_error(hl2_test(x = x[1:4], y = y))
  testthat::expect_error(hl2_test(x = x, y = c(y[1:4], rep(NA_real_, 10))))
  ...
  # Asymptotic test
  p.two.sided <- hl2_test(x = x, y = y, method = "asymptotic",
                          alternative = "two.sided")$p.value
  p.greater <- hl2_test(x = x, y = y, method = "asymptotic",
                        alternative = "greater")$p.value
  p.less <- hl2_test(x = x, y = y, method = "asymptotic",
  ...
  # Permutation test
  p.two.sided <- hl2_test(x = x[1:5], y = y[1:5], method = "permutation",
                          alternative = "two.sided")$p.value
  p.greater <- hl2_test(x = x[1:5], y = y[1:5], method = "permutation",
                        alternative = "greater")$p.value
  p.less <- hl2_test(x = x[1:5], y = y[1:5], method = "permutation",
                     alternative = "less")$p.value
  ...
  # computation of the right-sided p-value. Hence, it is counted twice than once
  # so that p.less + p.greater > 1
  perm.dist <- perm_distribution(x = x[1:5], y = y[1:5], type = "HL21", randomization = FALSE)
  hl21.statistic <- rob_perm_statistic(x = x[1:5], y = y[1:5], type = "HL21")$statistic
  testthat::expect_equal(p.less, 1 - p.greater + sum(hl21.statistic == perm.dist)/252)

  # Randomization test
  set.seed(168)
  p.two.sided <- hl2_test(x = x[1:10], y = y[1:10], method = "randomization",
                          alternative = "two.sided", n.rep = 10000)$p.value
  set.seed(168)
  p.greater <- hl2_test(x = x[1:10], y = y[1:10], method = "randomization",
                        alternative = "greater", n.rep = 10000)$p.value
  set.seed(168)
  p.less <- hl2_test(x = x[1:10], y = y[1:10], method = "randomization",
  ...
  testthat::expect_message(hl2_test(x = x[1:10], y = c(y[1:9], 0),
  ...
  # Comparison of wobbled values by 'hl2_test' and output of function 'wobble'
  set.seed(1234)
  wob <- wobble(x = x, y = y, check = FALSE)

  testthat::expect_equal(suppressWarnings(hl2_test(x = x, y = y,
                                                   method = "randomization",
                                                   n.rep = 1000,
                                                   wobble = TRUE,
                                                   wobble.seed = 1234)$statistic),
                         suppressWarnings(hl2_test(x = wob$x, y = wob$y,
                                                   method = "randomization",
                                                   n.rep = 1000,
